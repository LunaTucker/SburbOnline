...
function addPlayer(self, playerInfo, collisionLayer) {
  ...
  if (playerInfo.team === 'blue') {
      self.player = self.physics.add.sprite(playerInfo.x, playerInfo.y, 'karkat').play('kk_down_idle');
      self.playerPrefix = "kk";
      
  } else {
      self.player = self.physics.add.sprite(playerInfo.x, playerInfo.y, 'terezi').play('tz_down_idle');
      self.playerPrefix = "tz";
  }
  
    //and set up some player stuff
           //smaller collision box
      self.player.setSize(30, 70, true);
